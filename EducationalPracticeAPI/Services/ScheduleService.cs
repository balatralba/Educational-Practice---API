...
namespace EducationalPracticeAPI.Services
{
    public class ScheduleService : IScheduleService
    {
        ...
        public async Task<List<ScheduleByDateDto>> GetScheduleForGroup(string groupName, DateTime startDate, DateTime endDate)
        {
            ...
            StudentGroup? group = await GetGroupByName(groupName);
            List<Schedule> schedules = await LoadSchedules(group.GroupId, startDate, endDate);
        ...
        {
            StudentGroup? group = await _db.StudentGroups.FirstOrDefaultAsync(g => g.GroupName == groupName);
        ...
        private async Task<List<Schedule>> LoadSchedules(int groupId, DateTime start, DateTime end)
        {
            return await _db.Schedules
                .Where(s => s.GroupId == groupId && s.LessonDate >= start && s.LessonDate <= end)
                .Include(s => s.Weekday)
